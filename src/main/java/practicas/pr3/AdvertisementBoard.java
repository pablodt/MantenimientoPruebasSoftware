...
public class AdvertisementBoard {
  ...
  public void publish(Advertisement advertisement,
    ...
    if (numberOfPublishedAdvertisements() >= MAX_BOARD_SIZE && advertisement.advertiser.equals("Tim O'Theo")) {
      System.out.println("Entra");
      throw new FullBoardException();
    }

    if (findByTitle(advertisement.title) == null) {
      ...
      if (!findByTitle(advertisement.title).advertiser.equals(advertisement.advertiser)) {
        if (advertisement.advertiser.equals("THE Company"))
          advertisementList.add(advertisement);
        else {
          if ((advertiserDatabase.findAdviser(advertisement.advertiser)) &&
